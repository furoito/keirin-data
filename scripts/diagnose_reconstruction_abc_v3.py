#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""A/B/C reconstruction diagnostic with robust result-rank parsing.

This changes only result scoring. Pre-race selection and A/B/C candidate construction
are identical to v2. CSV rank values such as 1.0/2.0/3.0 are accepted as integer ranks.
"""
...
OUT=CTX/'reconstruction_abc_v3_diagnostic.csv'; SUMMARY=CTX/'reconstruction_abc_v3_summary.json'
...
def parse_rank(v):
    ...
    try:
        ...
        # tolerate common decorations around a numeric rank, but never infer
        # a finish position from non-numeric statuses such as 失/棄.
        import re
        m=re.fullmatch(r'\s*([1-9])(?:\.0+)?\s*',s)
        return int(m.group(1)) if m else None


def robust_actual_order(g):
    vals=[]
    ...
    # standard non-tied race only; ties are left unscored for this diagnostic.
    vals.sort()
...
def main():
    ...
    use=ctx.copy(); use=use[use.context_quality.astype(str)=='full']
    ...
    for cr in use.itertuples(index=False):
        ...
        bet_count+=1
        act=robust_actual_order(g)
        if not act:
            unresolved.append({'race_id':rid,'date':cr.date,'venue':cr.venue_slug,'race_no':cr.race_no,
                               'rank_values':'|'.join(str(x) for x in g['rank'].tolist()),
                               'san_ren_tan':'|'.join(sorted(set(str(x) for x in g.get('san_ren_tan',pd.Series(dtype=object)).dropna().tolist())))})
            continue
    ...
    pd.DataFrame(unresolved).to_csv(CTX/'reconstruction_abc_v3_unresolved.csv',index=False,encoding='utf-8-sig')
    hb=out[out.head_bust==1]
    s={'recomputed_bet_count':bet_count,'scorable_bet_count':len(out),'unresolved_count':len(unresolved),
