#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Minimal three-factor ticket test.

Purpose: test only the three highest-confidence structural ideas and remove
all discovered score-percentile / score-rank / score-order / score-gap cuts.

Factors:
1) ordered ticket 1st rider is a running_style='両' head of a multi-rider line
2) winner's own bante is excluded from the ticket
3) 2nd and 3rd riders are other-line bantes with race_score > winner's own bante

No group-score percentile restriction. No NO_THIRD rule is needed because
2nd/3rd are explicitly line_pos=2. Flat 1 unit per exact trifecta ticket.
Same discovery data: mechanism test, not OOS validation.
"""
...
OUT=CTX/'h1_minimal_three_factors_summary.json'
ODDS_BINS=[
...
def main():
    ...
    for month in h1.MONTHS:
        ...
        for cr in use.to_dict('records'):
            ...
            # Generate directly from structural factors, not from percentile-selected trios.
            for a in frames:
                ...
                # Need two distinct stronger other-line bantes to occupy 2nd/3rd.
                if len(stronger_other_bantes)<2: continue
                for b2,c3 in itertools.permutations(stronger_other_bantes,2):
                    # Require two different other lines; otherwise they are not two rival-line bantes.
                    ...
                    rows.append({
                        ...
                        'head_score':float(score.get(a,np.nan)),'own_bante_score':float(score[own]),
                        'second_score':float(score[b2]),'third_score':float(score[c3]),
                    })
    ...
    payload={
        'status':'exploratory_minimal_three_factor_ticket_test',
        'fixed_factors':[
            "ticket 1st rider = running_style 両, line_pos=1, multi-rider line head",
            "winner own bante excluded",
            "ticket 2nd and 3rd riders = bantes from two other lines, each with race_score > own bante",
        ],
        'explicitly_removed_filters':['group_score_percentile','NO_THIRD','ticket score rank','ticket score order','score gaps'],
        'warning':'Same discovery data. This isolates whether the three structural factors alone carry value; it is not OOS validation.',
        'usable_races_by_month':usable_by_month,'skipped':dict(skipped),
        'overall':agg(df),'ticket_odds_bins':slices(df),
        'min_ticket_odds':{str(c):agg(df[df.odds>=c]) for c in [50,100,200]},
    }
