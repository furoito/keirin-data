#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Long-history probe for the current baseline candidate.

Fixed candidate:
- exactly 3 distinct true lines
- selected lines have >=2 riders (single-rider 両 excluded upstream)
- each selected rider is running_style='両' OR line_pos=2 (bante)
- 0.20 < group_score_percentile <= 0.40
- no race-level top3-spread filter
- buy the single cheapest posted 3rentan permutation within the trio

Window: 2022-01 through 2026-08, using only available/usable monthly inputs.
"""
...
OUT = CTX / 'core_20_40_no_top3spread_long_history_summary.json'
DETAIL = CTX / 'core_20_40_no_top3spread_long_history_tickets.csv'
SEED = 20260904
...
def summarize(x: pd.DataFrame):
    ...
    for i, threshold in enumerate([30, 50, 100, 200]):
        cumulative[f'GE{threshold}'] = rb.summarize_slice(
            x[x.rank1_posted_odds >= threshold], SEED + 400 + i
        )
    payload['by_cumulative_posted_odds_threshold'] = cumulative
    return payload
...
def main():
    ...
    payload = {
        'status': 'exploratory_long_history_core_20_40_no_top3spread_probe',
        ...
        'candidate_definition': [
            'exactly 3 distinct true lines',
            'selected lines have >=2 riders (single-rider 両 excluded)',
            "each selected rider is running_style='両' OR line_pos=2",
            '0.20 < group_score_percentile <= 0.40',
            'no race top3-spread requirement',
        ...
        'candidate_exposure': {
            ...
            'group_hits': int(tickets.group_hit.sum()),
        },
        **summarize(tickets),
        'warning': 'Same-data exploratory analysis. Backfill coverage may still be changing; interpret period results with month coverage.',
    }
